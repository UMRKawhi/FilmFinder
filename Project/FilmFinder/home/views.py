...
def comments_delete(delete_id=None):
    user_id = session['login_user_id']
    comment = Comment.query.filter(Comment.id == delete_id, Comment.user_id == user_id).first_or_404()
    film = Film.query.filter(Film.id == comment.film_id).first_or_404()
    film.comment_num = film.comment_num - 1;
...
def search(page=None):
    ...
    return render_template('home/search.html', keyword=keyword, data_type=data_type, search_films=search_films, search_count=search_count)
...
def detail(id=None, page=None):
    ...
    if 'login_user' in session:
        user_id = session['login_user_id']
        blacklists = BlackList.query.filter(BlackList.user_id == user_id).all()
        banners = []
        for black in blacklists:
            banners.append(black.banner_id)
        page_comments = Comment.query.filter(
            Comment.film_id == film.id,
            Comment.user_id.notin_(banners)
        ).order_by(
            Comment.create_time.desc()
        ).paginate(page=page, per_page=10)
        temp_comments = Comment.query.filter(
            Comment.film_id == film.id,
            Comment.user_id.notin_(banners)
        ).all()
        score = 0
        ...
        comment_history = Comment.query.filter(Comment.user_id == user_id).order_by(Comment.score.desc(),
                                                                                    Comment.create_time.desc()).limit(
            10).all()
        if comment_history:
            score_greater = []
            score_middle = []
    ...
    print(page_recommendations.page)

    return render_template('home/detail.html', film=film, genres=genres, directors=directors, casts=casts, form=form,
                           page_comments=page_comments, page_recommendations=page_recommendations,
                           last_film_id=last_film_id)
...
def add_banner(banner_id=None, film_id=None):
    ...
    if banner_id and banner_id != user_id:
        user_id = session.get('login_user_id')
        blacklist = BlackList(
            user_id=user_id,
            banner_id=banner_id
